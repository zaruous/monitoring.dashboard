...
/**
 * 데이터 제공자를 관리하고 애플리케이션에 데이터를 제공하는 중앙 클래스입니다. Strategy Pattern의 Context 역할을 하며,
 * Singleton Pattern으로 구현되어 애플리케이션 전체에서 단일 인스턴스를 통해 데이터에 접근할 수 있도록 합니다.
 *
 * 사용된 디자인 패턴: 1. Strategy Pattern: DataProvider 인터페이스를 통해 데이터 소스를 동적으로 교체할 수
 * 있습니다. (예: SqliteDataProvider, MockDataProvider). 이를 통해 데이터베이스 구현의 변경이나 테스트
 * 환경으로의 전환이 유연해집니다. - Context: DatabaseManager - Strategy: DataProvider -
 * ConcreteStrategy: SqliteDataProvider, MockDataProvider
 *
 * 2. Singleton Pattern: 애플리케이션 전체에서 단 하나의 DatabaseManager 인스턴스만 존재하도록 보장합니다. 이를
 * 통해 데이터 제공자 설정(Strategy 설정)을 일관되게 유지하고, 어디서든 동일한 데이터 소스에 접근할 수 있습니다.
 */
public class DatabaseManager {
	...
	public List<InterfaceStatusDetail> getInterfaceStatusDetails(Parameter pathParamMap) {
		...
		return dataProvider.getInterfaceStatusDetails(pathParamMap);
	}
	...
	public List<InterfaceStatusDetail> getInterfaceStatusDetails(LocalDate date, INF_STATUS success) {
		if (dataProvider == null) {
			throw new IllegalStateException("DataProvider가 설정되지 않았습니다.");
		}
		return dataProvider.getInterfaceStatusDetails(date, success);
	}
